...
Table 1 shows the devastating effects of the lockdown on the restaurant industry. For restaurants whose income was less than the mean, these types of losses could quickly result in closure if intervention of subsidies are not put in place. Figure \@ref(fig:2020decfig) further represents this income reduction.  

```{r 2020decfig, fig.cap="Graph of average income and profit for all restaurants in the survey over a two month period between May and June", echo=FALSE, message=FALSE, warning=FALSE}
...
Table \@ref(tab:staff) shows a decrease in staff of more than 70%. These losses indicate mass unemployment in the restaurant industry and the far reaching implications of COVID-19, which will be discussed in depth in Section 3. Figure \@ref(fig:stafffig) gives a visual representation of the change in staffing. 
...
#Pricepoint
kbl(tabyl(all_pop_resturants$Price_Point_Target, sort = TRUE), booktabs = T, col.names = c("Price-Point", "Number of Restaurants", "Percent of Restaurants"), align = c("l", "l", "l"), caption = "Price-point of restaurants") %>%
  kable_styling(latex_options = c("striped", "scale-down"))
...
Within the low-end restaurants, reopening for a two month period showed significant improvements. Restaurants in the treatment group earned roughly 188% more than the control (Figure \@ref(fig:lowincome)). Employment showed more significant improvements with an almost 247% increase in staff (Figure \@ref(fig:lowstaff)). In the end, treated restaurants were torn on the helpfulness scale, with the average response at 3.36. Although 34% of respondents scored a five, 21% still scored a one (Table \@ref(tab:lowscore)). This could be due to the effort that it takes to prepare a restaurant to open for a two month period, especially in the wake of COVID-19. The amount of personal protective equipment (PPE), training and sanitizing equipment needed could outweigh the benefits of opening.

```{r lowincome, fig.cap="Profit of control and treatment groups of low-end restaurants between May and June of 2021", echo=FALSE, message=FALSE, warning=FALSE}
...
profit_sample_low_end <-ggplot(sample_resturants_low_plot_profit, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Treatment Low-End Resturants Profit"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("Profit May-June 2019", "Profit May-June 2020", "Profit May-June 2021"))

grid.arrange(profit_control_low_end, profit_sample_low_end, ncol=2, top = "Profit Comparision Between Low-End Resturants in Control and Treatment Group")
...
sample_low_end_staff <-ggplot(sample_resturants_low_end_plot_staff, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Treatment Low-End Resturants' Staff"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("# of Staff May-June 2019", "# of Staff May-June 2020", "# of Staff May-June 2021"))


grid.arrange(control_low_end_staff, sample_low_end_staff, ncol=2, top = "Staff Comparision Between Low-End Resturants in Control and Treatment Group")
...
Middle-end restaurants experienced fairly similar income outcomes compared to low-end restaurants. Restaurants in the treatment group saw a 179% increase in income in contrast to the control group (Figure \@ref(fig:midincome)). With this improvement, 38% of the restaurants categorized helpfulness as a five on the five point scale (Table \@ref(tab:midscore)). Although an additional 19% of restaurants scored a four, there were still almost 19% that scored a one. In addition, employment experienced a 254% increase in staff, which are similar numbers compared to low-end restaurants (Figure \@ref(fig:midstaff)). 


```{r midincome, fig.cap="Profit of control and treatment groups of middle-end restaurants between May and June of 2021", echo=FALSE, message=FALSE, warning=FALSE}
...
profit_control_average <- ggplot(control_resturants_average_profit, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Control Mid-End Resturants' Profit"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("Profit May-June 2019", "Profit May-June 2020", "Profit May-June 2021"))
...
profit_sample_average <-ggplot(sample_resturants_average_profit, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Treatment Mid-End Resturants' Profit"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("Profit May-June 2019", "Profit May-June 2020", "Profit May-June 2021"))

grid.arrange(profit_control_average, profit_sample_average, ncol=2, top = "Profit Comparison Between Middle-End Resturants in Control and Treatment Group")
...
control_average_staff <- ggplot(control_resturants_average_plot_staff, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Control Mid-End Resturants' Staff"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("# of Staff May-June 2019", "# of Staff May-June 2020", "# of Staff May-June 2021"))
...
sample_average_staff <-ggplot(sample_resturants_average_plot_staff, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Treatment Mid-End Resturants' Staff"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("# of Staff May-June 2019", "# of Staff May-June 2020", "# of Staff May-June 2021"))


grid.arrange(control_average_staff, sample_average_staff, ncol=2, top = "Staff Comparision Between Middle-End Resturants in Control and Treatment Group")
...
Finally, high-end restaurants experienced the smallest improvement in earnings, although by a fraction of a difference. The treatment group earned 178% more than the control group between May and June (Figure \@ref(fig:highincome)). The nature of high-end restaurants offering primarily dine-in service would assume that they would see the greatest positive effects of the reopening, yet they did not see as substantial of an increase as other price brackets. This could be due to lingering hesitancies surrounding COVID-19, or the number of people who experienced financial losses at this time. This increase in income could also be a factor in the 251% increase in staff (Figure \@ref(fig:highstaff)). Expectedly, high-end restaurants saw the lowest positive response to the reopening, with 33% scoring a five and over 20% scoring a one (Table \@ref(tab:highscore)).


```{r highincome, fig.cap="Profit of control and treatment groups of high-end restaurants between May and June of 2021", echo=FALSE, message=FALSE, warning=FALSE}
...
profit_sample_high_end <-ggplot(sample_resturants_high_plot_profit, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Treatment High-End Resturants' Profit"))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("Profit May-June 2019", "Profit May-June 2020", "Profit May-June 2021"))


grid.arrange(profit_control_high_end, profit_sample_high_end, ncol=2, top = "Profit Comparision Between High-End Resturants in Control and Treatment Group")
...
sample_high_end_staff <-ggplot(sample_resturants_high_end_plot_staff, aes(x = value, fill = key)) +
  geom_density(kernel = "gaussian", alpha = 0.1) + guides(fill=guide_legend(title="Treatment High-End Resturants' Staff"),
                                                          shape = guide_legend(override.aes = list(size = 0.5)))+ 
  theme(legend.title = element_text(size = 5), legend.text = element_text(size = 5), legend.position = c(0.82, 0.9))+
  scale_fill_discrete(labels = c("# of Staff May-June 2019", "# of Staff May-June 2020", "# of Staff May-June 2021"))
        
grid.arrange(control_high_end_staff, sample_high_end_staff, ncol=2, top = "Staff Comparision Between High-End Resturants in Treatment and Control Group")
